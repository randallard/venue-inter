<script lang="ts">
	...
	async function load() {
		...
		error = null;
		try {
			const data = await getAdminReviewQueue('disqualify');
			rows = data.rows;
		} catch (e) {
...
<div class="container">
	...
	<div class="page-header">
		<h1>Disqualification Requests</h1>
	...
	{:else if rows.length === 0}
		<div class="card empty">
			...
			<p>No disqualification requests pending.</p>
		</div>
	{:else}
				...
				<tbody>
					{#each rows as row}
						<tr>
							...
							<td>
								<a class="btn btn-primary btn-sm" href="/reviews/disqualify/{row.part_key}">
									Review
								</a>
							</td>
						</tr>
